...
router.post('/products/test', asyncHandler(async (req, res, next) => {
  const { name, description } = req.body;
  if (!name || name == "") return res.status(400).json({ status: "fail", statusCode: 400, message: "Name is required!" });
  if (!description || description == "") return res.status(400).json({ status: "fail", statusCode: 400, message: "Description is required!" });

  return res.status(200).json({ status: "success", statusCode: 200, message: `Name is ${name} and description is: ${description}` });
}))
